            ...
            {imagenSliders?.length > 0 ? (
              <Carousel slide={true} slideInterval={3000} indicators={false} className='w-full h-full px-12'
                ...
                {imagenSliders?.map((imagenSlider, i) => (
                  <Image key={i} width={1340} height={500} src={imagenSlider?.file} loader={imageLoader} priority={true} objectFit="cover" layout="responsive" className="block w-full h-full" alt=""/>
                ))}
              </Carousel>
              ) : null}
          </Suspense>
