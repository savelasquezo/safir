  ...
  useEffect(() => {
    const fetchImagenSliders = async () => {
      try {
        ...
        setImagenSliders(res.data.results);
      } catch (error) {
        ...
        <div className={`overflow-hidden relative z-0 h-44 md:h-80 lg:h-[calc(100vh-112px)]`}>
          <Suspense fallback={null}>
            {imagenSliders?.length > 0 ? (
              <Carousel slide={true} slideInterval={3000} indicators={false} className='w-full h-full'
                ...
                {imagenSliders?.map((imagenSlider, i) => (
                  <Image key={i} width={1895} height={810} src={imagenSlider?.file} loader={imageLoader} priority={true} className="w-screen h-full bg-contain" alt=""/>
                ))}
              </Carousel>
              ) : null}
          </Suspense>
